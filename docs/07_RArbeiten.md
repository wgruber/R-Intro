...
```r
  load("Data/bigfive.RData") # A1
  KopieBigFive <- bigfive  # A2
  save.image("Data/BigFive_1.RData")  # A3
  rm(list = ls()) # A4
  load("Data/BigFive_1.RData") # A5
  save.image("Data/BigFive_2.RData") # A6
  rm(list = ls()) # A7
  load("Data/BigFive_2.RData") # A8
  
  saveRDS(KopieBigFive, file = "Data/BigFive_3.rds")
  rm(list = ls())
  KopieBigFive <- readRDS( "Data/BigFive_3.rds") 
```
...
```r
  ...
  save(Seminar, V1, file = "Data/BigFive_4.RData") 
  rm(list = ls())
  load("Data/BigFive_4.RData")
  rm(list = ls())
```
...
```r
    WebData <- read.table("http://www.tuval.at/wp-content/uploads/Airquality.txt",
                          sep = ",",
                          header = FALSE)
```
